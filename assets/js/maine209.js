(function ($) {
    ...
    var _container_left = 0;
    if (window.innerWidth >= 992) {
        var titleFleet = $(".container");
        if (titleFleet.length > 0) {
            _container_left = titleFleet.offset().left + 15;
        }
    }
...
// Dynamic dropdown text updater for search bar
$(document).ready(function() {
    $('.dropdown-menu a.dropdown-item').on('click', function(e) {
        e.preventDefault();
        var selectedText = $(this).text();
        $(this).closest('.dropdown').find('.btn-dropdown-search').text(selectedText);
    });
});
